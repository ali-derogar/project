...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                "دیدگاه برای شما"
                , "دیدگاهی برای مقاله شما {}با لینک زیر ارسال شد \n \
                {}{}".format(article ,current_site,reverse("web:detail" ,kwargs={"slug":article.slug}))
                , to=[author_email])
            email.send()
        if user_email:
            email = EmailMessage(
        ...
        if parent_email:
            email = EmailMessage(
                "پاسخ به دیدگاه شما"
                , "دیدگاهی برای پاسخ به دیدگاه شما {}با لینک زیر ارسال شد \n \
                {}{}".format(article ,current_site,reverse("web:detail" ,kwargs={"slug":article.slug}))
                , to=[parent_email])
            email.send()
